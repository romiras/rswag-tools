...
var
   ...
   FileOpened,
   UuStarted,
   ItIsUUE:Boolean; { è‡®ß≠†™ UUEncoded Stuff }
begin
...
{   end;}
   ...
   UuStarted:=False;
   FileOpened:=False;
   ItIsUUE:=False; { è‡®ß≠†™ UUEncoded Stuff}
...
{ ƒƒƒƒ Draw the Record to Full Screen ƒƒƒƒ }
{Var
   cID:PathStr;
   nLen:byte Absolute cId;
   cArea:string;
...
{ ƒƒƒƒ Draw the Record Header to Full Screen ƒƒƒƒ }
...
{   end;}
   ...
   If FieldExists('PROJECT') then begin  {Skip this if old DBF format}
      SetColor('B+/N');
      SAY(52,5, 'ƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒƒ');
      SetColor('W/N');
...
begin
 ...
 If Alias = 'WORK' then begin  { éØ•‡†Ê®Ô ¢Æß¨Æ¶≠† ‚Æ´Ï™Æ ®ß Msg List }
   SaveColor(OldColor);
   OldScreen:=SaveScreen;
...
{--------- Check Ini Parameters ----------------------}
   ...
   Config(ConfigFile); { ¢ßÔ‚Ï Ø†‡†¨•‚‡Î ≠†·‚‡Æ©™® ®ß ‰†©´† ™Æ≠‰®£„‡†Ê®® }
