...
begin
   ...
   If l + Memo_Size > MaxMemoSize then begin
      ...
      {!!! Предложить разбить на несколько секций!!!}
   end else begin
      for j:=Pred(Memo_Size) downto p do  {Сдвинуть Хвост Memo вправо}
         memo^[j+l]:=memo^[j];

      {вставить нашу строку в освобожденное место}
...
begin
  ...
  While (crs < Memo_Size) do begin
    crs := scanCrDn(crs);
    If (Memo^[crs-1]=K_LF) and  (Memo^[crs-2]='Н') then begin
       Dec(crs,2);
       DelChar;
       DelChar;
       MemoUpdated:=True;
...
begin
 ...
 Repeat
   ...
   If    ScreenNum = 1       then SAY(69,24,'───── PgDn ')
   else
      If ScreenNum = LastNum then SAY(69,24,'───── PgUp ')
...
*)
      K_F3,
      K_Alt_H      : begin {Remove 'Н-LF'    }
                        RemoveSoftCR;
                        If MemoUpdated then
                           SetCursorOn;
                     end;
      K_Alt_K      : begin {Remove 'Н-LF'    }
                        ChangeCutters;
                        If MemoUpdated then
                           SetCursorOn;
                     end;
